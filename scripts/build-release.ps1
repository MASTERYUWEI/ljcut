...
# 注意：PowerShell 無法把環境變數設成空字串、外部指令引號又會被改寫，
# 簽章需 PASSWORD 存在且為空 → 寫成暫存 .sh 交給 Git Bash 執行（繞開兩層引號地獄）。
Push-Location $Root
$sh = Join-Path $env:TEMP "ljcut-build.sh"
@(
    'set -e',
    'export TAURI_SIGNING_PRIVATE_KEY="$(cat ~/.tauri/ljcut_updater.key)"',
    'export TAURI_SIGNING_PRIVATE_KEY_PASSWORD=""',
    'npx -y @tauri-apps/cli build'
) | Set-Content -Path $sh -Encoding ascii
& "C:/Program Files/Git/bin/bash.exe" $sh.Replace('','/')
